...
func _ready() -> void:
	...
	if scene_file_path:
		shell_path = scene_file_path.to_lower()
	
	var check_map: Dictionary = {"green": 0, "red": 1, "blue": 2, "yellow": 3}
	for key in check_map:
		if key in shell_path:
			set_meta(&"shell_color_id", check_map[key])
			break
...
func _physics_process(delta: float) -> void:
	...
	if stopping:
		speed.x = 0
		ShellManager.register_shell(self);
...
func status_update() -> void:
	...
	if !stopping:
		animation.play()
		ShellManager.unregister_shell(self)
